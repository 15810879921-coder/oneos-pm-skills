...
- 凡是读取、判断、引用或执行上述五个云效生命周期 Skill，必须先运行每日更新器，再从用户级安装目录重新定位并读取当前 `SKILL.md`；不得用会话缓存、源码工作树、发布说明或更新前读取结果冒充本机当前版本。
- `updated` 之后必须重新加载当前 Skill 及本次所需引用。`skipped-today` 只证明今天曾成功运行更新器，不证明后续发布的新版本已经安装；当本轮已有远端新提交、新发布版本、源码与安装目录不一致或用户报告版本落后等新鲜证据时，立即运行 `node <本 Skill 目录>/scripts/ensure-daily-skill-update.mjs --current-skill development-brain --force`，不得等待次日。
- `in-progress` 时不得报告当前版本或开始云效生命周期写入；等待现有更新结束后再回读安装目录。`failed`、`unavailable` 或 `cooldown` 不阻断普通本地开发，但在无法完成安装回读时，不得声称“已是最新版本”，也不得以未核验套件版本执行新格式生命周期写入。
- 对外报告当前版本前，回读本次实际使用的 Skill 安装路径、明确版本号和文件修改时间；仅在用户要求检查完整套件安装时盘点五个 Skill。开发台账和完成开发不再以其他岗位 Skill 缺失或版本不一致阻塞，也不运行五 Skill 同版门禁；仍须核验实际输入协议、交棒及业务证据，不能仅依据更新命令退出码宣称更新成功。
...
版本：`10.2.12`。自然语言和当前工作区表明开发已经开始、正在继续或刚刚完成时，先完整读取 [主动识别与自动进化](references/semantic-evolution.md)，不要求用户额外输入 Skill 名称或固定口令。
...
- 正式云效开发的本次产品合同、理解回执、工程决定与验收证据属于当前任务，由 `yunxiao-development-delivery` 的版本化交棒门禁核验；预检通过、历史经验或本机分身能力不能替代它们，更不能把工程未知补成产品已确认。只传正式编号和证据链接，不要求同事复制产品经理个人记忆/分身。此条不改变知识准入、自动进化或生产授权规则。
