...
skill-run yunxiao_cli_allocate_task.py preflight \
  ...
  --requirement-snapshot-file <需求快照.md> \
...
预检前先执行`skill-run yunxiao_cli_allocate_task.py analyze --task ...`读取正式关联需求。按`requirement-snapshot.md`优先读取 PM 产品交棒快照并裁剪当前任务范围；历史无快照时从权威引用材料兼容补录并标记。先生成任务快照文件，再按`allocation-technical-plan.md`生成绑定快照哈希的方案；预检命令必须同时传入两个文件。预检必须通过CLI完成并输出带哈希的系统临时目录回执：

1. 根据任务编号前缀唯一解析项目，并按精确编号读取`【交付】`。
2. 验证交付负责人已明确且可官方回读、状态属于`待处理|已分配|处理中`且未关闭；不限定固定姓名。
...
9. 校验日期和正数工时；校验快照绑定关联需求编号并含任务目标、本次范围、页面与交互、业务规则、验收条件、来源与版本；校验技术方案绑定同一需求及快照哈希，并含实现范围、处理逻辑、实施步骤、验证标准。冻结需求描述哈希、快照哈希、方案哈希、对象、关系、可选优先级、字段、状态和负责人指纹；来源优先级缺失时记录告警，不猜值、不阻塞。
...
执行器必须先重新预检。工作项、关系、负责人解析、字段配置、状态配置、快照文件或方案文件任一变化时拒绝写入并要求重新预检。

未漂移时按固定顺序执行：

1. 创建或复用唯一`【开发】<需求标题>`；创建时使用`parent-id`直接归入来源交付。来源交付有`priority`时把标识ID原样写入开发任务并回读；缺失时不传优先级并记录告警。复用已有开发任务时保留其现有优先级，不做覆盖。
...
3. 开发需求快照和技术实施方案仅保留在本地并由回执记录其哈希；不得写入、重写或清理云效开发任务描述。
4. 通过CLI创建或复用`PARENT→交付`和`ASSOCIATED→需求`，逐条回读。
5. 聚合回读开发任务负责人、日期、工时、状态、两条关系及本地快照、方案哈希；仅当来源交付有优先级时，要求新建任务的优先级标识ID与其完全一致。
