...
产品交接是死门禁（10.2.13）：`evidence.handoffEvidence`、实时产品 manifest、必读资料哈希、开发理解回执、范围与交付版本全部通过后，才能执行本次流程任何写入。缺失、损坏、过期、不一致或不可读均停止，不降为警告；旧预检、历史已提交代码、已有测试任务、人工状态和催办确认都不能豁免。即使发生在可选工时阶段的写前校验中，交接失败也不得被当作工时失败跳过。补齐资料后重新预检。
...
测试流水线的实际执行不由阶段事务直接写入；`testDeliveryMode=execute`只输出下一动作，由外层调用独立的`执行测试流水线`命令。该命令必须把当前交接包中的项目、仓库/组件和目标分支写成冻结范围，先运行`discover_test_pipelines.py`列出候选，核验逻辑测试环境、上次成功部署提交SHA和Codeup待部署变更，只有唯一`result=ready`回执才可执行并回读实际部署版本。`merge_only`不读取或启动流水线。迭代、正式QA结论和需求`测试完成`不属于本执行器。
...
计划使用`oneos.complete-development-plan/v1`，套件版本为`10.2.13`。生成计划前必须通过只读CLI（TestHub计划读取默认使用受控官方JSON适配器）冻结以下事实：
