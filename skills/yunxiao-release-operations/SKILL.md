...
Operate deployment and release evidence while keeping test, production verification, rollback, and product acceptance separate. Suite version: `9.3.0`.
...
8. Create/reuse one exact top-level `【发版】` task directly from the project, iteration, sorted A-class requirements, same-iteration completed Bugs without delivery relations, and scope hash. Formally relate it to the iteration, A-class requirements, their source delivery/test tasks and qualifying Bugs; write and read back the immutable A/B/C/D scope, relation lists, available test-record summary, production pipeline, owner, approver, window, rollback plan, and production-verification plan. Existing frozen scope drift blocks rather than being silently expanded.
