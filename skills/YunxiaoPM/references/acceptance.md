...
| 11 | 产品验收 | 以顶层发版任务唯一锚定；生产证据、需求/本批源交付与无交付已完成Bug关联均已校验；通过后关闭本批交付与发版任务；双端需求仅当全部端侧交付已完成才关需求，否则保持发布完成（部分验收） |

设计完成额外：AutoPRD 段已写；交付非占位（除非失败停下）；ZIP+截图已挂或已列出缺项。

迭代额外：只挂【交付】（需求不挂）；版本号符合递增规则。

生产后产品验收额外：按[release-acceptance.md](release-acceptance.md)检查prod执行、验签回调、生产验证、完整观察窗口、不可变生产锚点、顶层发版任务与scope/本批源交付/无交付已完成Bug的正式关系；`accept_release.py`先`--dry-run`。验收通过后本批【交付】和发版任务回读`已完成`；双端需求若对端交付未完成则需求保持`发布完成`并回报`partialAcceptance`，两端齐备后才关需求；无交付已完成Bug只回读验收证据，部分成功按同一幂等键安全续跑；验收不通过时写统一失败证据并输出正式修复回流口令。
