...
7. 写云效分级：建需求、批量推进、取消、验收、创建迭代仍须 **Plan → 确认 → apply**；Web 整批验收 Plan 必须同时列出来源迭代及连续两期子版本补建，验收确认同时覆盖关闭和补建，不二次确认；同一已授权批次中，明确编号且不扩范围的幂等刷新、续跑、补回读不重复确认
...
3. Plan 写清：项目名+spaceId、需求/任务编号、将改状态、负责人及来源、交付·分析·设计编号策略、产品快照编号/哈希或占位缺失风险、迭代类型（若有）、**不会做的事**（不建【开发】/【测试】、不按标题查重）。Web 整批验收还须列出从本批交付回读的冻结来源迭代、后两期名称/日期/负责人及预检哈希；已确认非 Web 明确`skippedNonWeb`，来源未知明确`unresolved/missing_source`并要求补来源信息。记录需求仅对尚未由明确参数唯一锁定的项目/类型/优先级/标签展示 [compact-select.md](references/compact-select.md) 选择题。
...
验收通过：发版任务=TASK-900；验收人=…；证据=…；[来源迭代ID=仅无法从本批工作项唯一回读时提供]
...
2. 生产后：按发版任务执行产品验收（**不以**发布生产证据区块为前置硬门；范围支持发版→需求或发版→【交付】→需求）。未指定子范围时验收冻结清单全部范围；指定组件/部署目标时只写该子范围验收事件，不关闭整批需求/交付/发版任务，并输出精确分支清理候选。全部范围通过后逐项、幂等地关闭需求、交付容器和发版任务；若本批唯一来源是 Web 版本迭代，所有必要关闭回读成功后按冻结基线自动补建连续两期空 Web 子版本。重复验收只续跑同一冻结目标；补建失败单独报告可续跑，不回滚或改称验收失败。小程序、BI/非版本迭代、局部或不通过均不补建。部分成功时重试只续跑未完成对象；不通过则记录统一证据、尝试将发版任务标为发布失败并正式交给测试侧发起修复回流。
