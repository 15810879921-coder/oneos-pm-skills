...
满足任一即进入确认流程（须 `SwitchMode` → **plan**）：

- 用户说：`确认待确认` / `AutoRDO 确认` / `逐条确认` / `消解待确认` / `补充确认`
- 用户对刚输出的清洗稿回复「确认」「开始确认」等
- 用户粘贴补充说明，要求据此消解待确认

**不进 Plan**：仅出清洗初稿、只问「什么意思」、不消解待确认点。

## Plan 门禁（强制）

1. `SwitchMode` → **plan**，说明：逐条确认待确认项，确认后再回填定稿；本阶段仍**不写云效**
...
| 优先级 | 方式 | 适用 |
|---|---|---|
| 1 | **选择题**（2–5 个互斥选项 + 可选「其他/我补充」） | 模块归属、部门名称、是否/范围、枚举字段、拆条边界等 |
