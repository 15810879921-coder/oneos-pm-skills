...
| 对象 | 动作 |
|---|---|
| 需求 | 先收口至设计完成；全部端侧冻结资料通过 `preflight-handoff / apply-handoff` 后才推进 **待开发** |
| 【交付】 | **必须**用交付任务编号定位；负责人取命令或项目配置的唯一解析结果并回读；不改计划开始；不写计划完成；描述仍占位则保持（除非同时设计完成+原型+AutoPRD） |
...
仍不创建【开发】/【测试】。占位仅允许调查/补录，不可正式交棒；见 [handoff-and-rollback.md](handoff-and-rollback.md)。
