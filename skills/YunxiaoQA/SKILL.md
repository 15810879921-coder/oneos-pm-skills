...
description: >-
  ...
  “完成测试”也可由测试人员显式给出“人工确认通过=是”进入审计放行分支，将【测试】标已完成、需求推进测试完成，但标记为非正式发布候选。
...
`完成测试`中显式的`人工确认通过=是`是上述普通门禁的唯一例外：不要求部署证据、TestHub计划/用例、QA manifest、缺陷关闭或范围聚合；直接将当前【测试】推进`已完成`、需求推进`测试完成`。必须把当前登录测试用户、时间、说明、幂等键、跳过项和关联缺陷快照写入`oneos.qa-manual-complete/v1`并回读；关联Bug保持原状，禁止伪造成普通测试证据，交接标记为非正式发布候选。
