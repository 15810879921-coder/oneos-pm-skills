...
Operate deployment and release evidence while keeping test, production verification, rollback, and product acceptance separate. Suite version: `10.2.15`.
...
- Accept and emit only formal Skill names, exact requirement/test/release/execution task IDs, live states, formal relations, frozen batch scope, and necessary pipeline, deployment, verification, rollback, evidence, or idempotency identifiers. Every affected scope must carry one current `oneos.handoff-evidence/v1`; oral handoff, a green build, prototype acceptance, or a completed work-item state is never a substitute.
...
Partition the batch by delivery tag (`Web` / `小程序`; `PC` aliases to `Web`). It selects every non-deferred requirement at `测试完成` and validates A/B/C/D, `testMode`, and one current formal handoff bundle per development scope. Every non-cancelled development task requires exactly one completed mapped test task. `formal-plan`, `mandatory-test-task`, and legacy `qa-requested-exception` may qualify; `lightweight-verification` is rejected. Any C/D or handoff gap is report-only and must not create/update a cloud release draft. Only A-nonempty/C-D-empty is marked `准备完成` and eligible for `执行发布`. Identity ambiguity, cross-project scope or conflicting source ownership remains zero-write.

**Web** validates/backfills `【代码交付记录】` against Codeup and freezes a complete repository-to-component-to-production-pipeline matrix. **小程序** records a per-delivery `miniprogram_skip_pipeline` channel and does not require cloud production pipelines or Codeup anchors. The business description and append-only managed ledger remain separate. `准备发布` never reads or starts a test pipeline. Repeating the same valid scope reuses the same task; it does not change scope, execute production, or change release state.

For every selected development scope, preparation must run the bundled `handoff_gate.validate_bundle(..., "release")`. The bundle must match the exact project/requirement/delivery/scope and delivery version, contain current development and QA receipts, have no engineering decision that blocks release, and contain `qaResult.formal=true` with exactly one evidenced `PASS` for every required acceptance ID. `formal:false`, `NOT_RUN`, an old manifest hash, a missing receipt, or a token/deadline rationale is a C-class blocker. Preparation may report these gaps but must not write a release draft until all affected scopes pass.
...
1. For a direct production pipeline command, use the supplied name to narrow existing definitions, then verify the exact Codeup source, branch, deployment target and logical environment before execution; the name itself is not environment proof. For `执行测试流水线`, first run `discover_test_pipelines.py` against the frozen project/repository/component scope. It must enumerate every page of `flow-list-pipelines`, read each matching definition and all relevant run pages, verify the logical test environment and project/source/branch/deploy-target identity, then read the last successful run's actual deployed commit and calculate Codeup commits after that baseline. A candidate name or ID only selects among already-verified candidates. Any missing baseline, incomplete Codeup history or non-unique candidate blocks execution. Before any release matrix selection, enumerate every page of `flow-list-pipelines` until its terminal page; never treat page 1 as the full pipeline inventory. `准备发布` never reads a test pipeline. `执行发布`、`执行回滚`、`重新发布`和`查询发布` only read production candidates whose code source, verified target branch, deployment target and logical environment consistently identify production. Only explicit `执行测试流水线` may read and run a test pipeline. Exclude disabled/archived and retired definitions before uniqueness checks. Treat Flow's generic `envName=日常环境` as a container label, not as production truth; record both the raw environment and derived logical environment. Never create, copy, update, rename, or delete a pipeline.
...
11. Initialize one `releaseMergeAttemptId` from the frozen plan with `execute_release_merge_plan.py init`. Preflight every repository target HEAD, exact source commit set, MR state, mandatory checks, clean-candidate patch tree and pipeline mapping before the first merge. When replay or the candidate MR reports a conflict, keep the same execution authorization and run the deterministic conflict workflow in [references/change-coverage.md](references/change-coverage.md); do not stop merely to request another `执行发布`. Persist base/ours/theirs, resolution, full replay, range diff, compile/test and candidate-tree proofs, create a reasoned plan revision, re-run drift/coverage, and continue. The candidate revision is proof only: retain the original frozen sources/histories and never rescan candidate history as a new business source. Persist each successful target merge immediately with its read-back target SHA. If a later repository fails, record `PARTIAL_TARGET_MERGE`; do not undo already merged commits silently, do not rebuild a new plan without a revision reason, and do not start production. Resume only the still-pending repositories after revalidating all target HEADs, or use separately reviewed revert MRs and record `REVERTED` after every occupied target is restored.
...
- The formal `oneos.handoff-evidence/v1` bundle is mandatory for every released scope. Its current manifest/receipts, delivery version and per-acceptance `qaResult` are release gates. Older auxiliary QA/test-deployment/defect JSON, approver fields, release windows, rollback plans, callbacks and production-verification plans remain optional records and do not replace or weaken the formal bundle.
...
- Release preparation validates this Skill and the actual input protocol compatibility. Do not require all five lifecycle Skills to be installed or share a suite version, and do not inspect another Skill installation directory. Preserve current scope, code version, formal QA and production authorization gates; incompatible evidence blocks only its dependent action, with the incompatible field and supported contract stated explicitly.
- No production pipeline may start while any repository in the frozen `mergePlan` is pending, failed, drifted, or reverted incompletely. A deterministically resolvable conflict must be resolved, tested, recorded in a reasoned new plan version and continued under the same execution; only unresolved or unsafe conflicts block. A conflict never authorizes a force merge.
