...
def first_value(value: Any, keys: set[str]) -> str:
    if isinstance(value, dict):
        for key, child in value.items():
            if str(key).lower() in {item.lower() for item in keys}:
                candidate = text(child)
                if candidate:
                    return candidate
            candidate = first_value(child, keys)
            if candidate:
                return candidate
    elif isinstance(value, list):
        for child in value:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if not args.no_monitor:
        monitor_result = monitor(pipeline_id=pipeline_id, run_id=run_id, executable=executable,
    ...
    action_result = (applied.get("actions") or [{}])[0].get("result")
    initial_readback = (applied.get("verifications") or [{}])[0].get("value")
    print(json.dumps({"result": (monitor_result or {}).get("result", "started"),
                      "pipelineId": pipeline_id,
                      "pipelineName": text(result["candidates"][0].get("pipelineName")),
                      "environment": text(result["candidates"][0].get("environment")),
                      "pipelineRunId": run_id,
                      "url": first_value(action_result, {"url", "webUrl", "pipelineRunUrl"}) or None,
                      "initialStatus": run_status(initial_readback) or None,
