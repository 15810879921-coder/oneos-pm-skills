...
class ReleaseComponentMatrixTests(unittest.TestCase):
    ...
    def test_new_contract_requires_valid_ledger_and_bidirectional_anchor_fields(self):
        ...
        self.assertEqual(errors, [])
        self.assertEqual(result["suiteVersion"], "10.2.0")
    ...
    def test_current_10_2_2_matrix_is_supported(self):
        value = matrix()
        value["suiteVersion"] = "10.2.2"
        value["ledgerValidation"] = {"status": "passed"}
        value["codeAnchors"][0].update(
            deliveryUnitId="DU-1",
            branchInstanceId="BR-1",
        ...
        errors, result = MODULE.validate(value)
        self.assertEqual(errors, [])
        self.assertEqual(result["suiteVersion"], "10.2.2")

    def test_current_10_2_3_matrix_is_supported(self):
        value = matrix()
        value["suiteVersion"] = "10.2.15"
        value["ledgerValidation"] = {"status": "passed"}
        value["codeAnchors"][0].update(
            deliveryUnitId="DU-1",
            branchInstanceId="BR-1",
        ...
        errors, result = MODULE.validate(value)
        self.assertEqual(errors, [])
        self.assertEqual(result["suiteVersion"], "10.2.15")
