...
| 交付端=PC\|小程序 却出现多条交付，或交付端=共用服务 却不是恰好 PC+小程序 各一 | 列出编号请人合并/补齐后再继续 |
| 功能模块标签用「分析/设计/交付」 | 模块标签打在需求（可选交付）上 |

## 任务标题（仅展示）

- 单端：`【交付】` + 需求标题；双端：`【交付】【PC】` / `【交付】【小程序】` + 需求标题
- 单端：`【分析】` / `【设计】` + 需求标题；双端：对应加 `【PC】` / `【小程序】` 前缀
- **端侧标签（强制）**：每条【交付】建单后打显示名 `PC` 或 `小程序`（与该条所属端一致）；可与需求模块标签并存。单端、双端均须打，避免列表肉眼混淆。详见 [settled-rules.md](settled-rules.md)
- 标题可改，**不参与查重**。
