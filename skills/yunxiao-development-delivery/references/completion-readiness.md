...
- `recoveryNeeded`还可包含`formal_handoff`和`managed_test_scope`。它们表示可通过补录恢复的历史缺口，不代表门禁已经通过；无法恢复或已确认冲突仍写入`knownBlockers`。
...
- `recovery_required / recover_then_complete`：正式历史任务已经找到可核验候选，但交棒、代码映射或既有测试任务的受管范围尚未补齐。展示精确候选集合；代码归属无法由任务编号、已有台账或正式关系自动证明时只确认一次。确认后按[历史代码交付恢复](historical-code-recovery.md)补录并重新分类，不能直接关单。
