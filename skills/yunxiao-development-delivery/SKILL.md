...
7. Resolve exactly one project user with role `测试主管`, create or reuse the unique `【测试】` task, force its owner to that user's ID, verify the role-owner match, move the requirement from `开发完成` to `待测试`, and produce the formal test handoff.
...
- Creating or reusing `【测试】` and moving the requirement `开发完成 → 待测试` are two separate verified actions. Exactly one project `测试主管` and a read-back equality check between the task owner user ID and supervisor user ID are mandatory test-handoff gates.
