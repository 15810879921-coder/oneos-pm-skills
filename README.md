...
| `AutoRDO` | 将碎片化聊天记录、语音转写文本拆解为清晰标题与书面描述；多行/多条自动拆成多份结果；对齐 OneOS 标准模块/部门用词（`oneos-domain.md`） | 见下方 |
...
3. AI 先判断是否多条：是则按条拆分；再对齐 OneOS 领域词典，输出一份或多份 **标题** + **描述**（无结尾句号），并梳理待确认事项。
