...
```text
创建迭代：交付端=PC；版本类型=副；交付任务=ONEOS-a,ONEOS-b,ONEOS-c；名称前缀=统一运营管理平台PC端
创建迭代：交付端=小程序；版本类型=子；交付任务=ONEOS-99；名称前缀=统一运营管理平台小程序
```

| 参数 | 规则 |
|---|---|
| 交付端 | **必选**：`PC` / `小程序`；未给则停下询问，禁止默认猜 |
...
`交付端=共用服务` 的需求会有两条端侧【交付】：创建 PC 迭代时只挂 `交付(PC)`；创建小程序迭代时只挂 `交付(小程序)`。禁止把两条交付挂进同一端侧迭代。
