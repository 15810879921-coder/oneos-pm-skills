...
| 11 | 产品验收 | 以发版任务唯一锚定；生产证据已校验；通过才关闭需求/交付/发版任务，不通过只写证据 |

设计完成额外：AutoPRD 段已写；交付非占位（除非失败停下）；ZIP+截图已挂或已列出缺项。

迭代额外：只挂【交付】（需求不挂）；版本号符合递增规则。

生产后产品验收额外：按[release-acceptance.md](release-acceptance.md)检查prod执行、验签回调、生产验证、完整观察窗口、不可变生产锚点、scope与正式需求关系；`accept_release.py`先`--dry-run`。验收通过后需求、对应【交付】和发版任务均回读`已完成`，部分成功按同一幂等键安全续跑；验收不通过时写统一失败证据并输出正式修复回流口令。
