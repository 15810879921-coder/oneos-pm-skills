...
| `AutoRDO` | 清洗为标题+描述；多行拆多条；待确认可 Plan 逐条确认（词典辅助选择题 + 文字补充） | 见下方 |
...
将碎片化文字、聊天记录或录音转写文本，在**保留原意**前提下自动拆解为清晰**标题**与**描述**，供入库写入云效需求。清洗 ONE-OS 材料时会先读 `references/oneos-domain.md`，对齐标准模块名、部门名与跨模块联动口径（词典本身不写入输出稿）。  
一次粘贴**多行 / 多条**独立诉求时，自动拆成多份转译结果（一份需求一份稿），不合并成一条。  
清洗稿中的「待确认」可说「确认待确认」进入 **Plan 模式逐条确认**：优先选择题（选项参照 OneOS 词典），也可用补充需求描述一次消解多点，再回填定稿。

### 何时使用

- 收到碎片的聊天记录、会议速记、录音转写稿时
- 记录需求到云效前，将需求材料拆解与清洗为标准标题与描述
- 一次粘贴多条待入库清单（每行一条、编号列表等）
- 清洗后消解「待确认」点（选择题或文字补充）
- 口令：`AutoRDO：<粘贴聊天或转写>` / `确认待确认`

### 怎么用

1. 在 Cursor 或 Codex 终端粘贴安装命令并执行（或把「发给 AI」文案粘贴给 Agent 代装）。
2. 对 AI 说：`AutoRDO：<粘贴聊天记录或转写文本>`；多条时直接多行粘贴即可。
3. AI 先判断是否多条：是则按条拆分；再对齐 OneOS 领域词典，输出一份或多份 **标题** + **描述**（无结尾句号），并列出待确认事项。
4. 需要定稿时说：`确认待确认`（或从第 N 条开始）。Agent 切 Plan，逐条出选择题；也可粘贴补充说明消解。确认后回填「已确认」定稿。
5. 定稿后再交 YunxiaoPMapp 按条记录需求（AutoRDO 本身不写云效）。
