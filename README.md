...
| `AutoRDO` | 清洗为标题+描述；多行拆多条；**有待确认则强制进 Plan** 逐条确认（词典辅助选择题 + 文字补充） | 见下方 |
...
清洗结果中**只要存在「待确认」**，Agent **同轮强制**进入 Plan 逐条确认（优先选择题，选项参照 OneOS 词典；也可用补充需求描述消解），**无需**再说「确认待确认」。无待确认则直接定稿。
...
3. AI 先判断是否多条：是则按条拆分；再对齐 OneOS 领域词典，输出一份或多份 **标题** + **描述**（无结尾句号）。
4. **有待确认则同轮自动切 Plan**：逐条出选择题；也可粘贴补充说明消解。确认后回填「已确认」定稿。无待确认则跳过 Plan。
