...
```text
记录需求：…；交付端=Web|小程序|共用服务；优先级=紧急|高|中|低；标签=…；提交部门=…；提交人=…；推进至=…
```

`交付端` 必选：`Web` / `小程序` / `共用服务`；未给则停下询问，禁止默认猜。  
含义（产品确认）：`Web`/`小程序` = 后续只建对应单端【交付】；`共用服务` = 开始分析或快轨时建 **双端【交付】**（Web+小程序 各一，挂同一需求）。  
其余四字段（优先级/标签/提交部门/提交人）均可选出现；网页或 AutoRDO 提示里已带的值与口令等价。

## Plan 是否追问

记录需求仍须确认创建计划。类型、优先级、标签按 [compact-select.md](compact-select.md) 点选；若口令给出的项目 ID 已被官方唯一回读，项目题直接锁定并从压缩串省略，不再要求人工点选项目。

| 条件 | 行为 |
|---|---|
| 口令已含类型/优先级/标签 | 对应选项标 ★ + 建议压缩串；仍须确认创建计划 |
| 口令已含项目 ID且官方唯一回读 | 回显锁定的项目名+ID；不生成项目选择题、不要求再次确认项目 |
...
| **快轨标签** | 【交付】【设计】建单后 `PATCH propertyKey=tag`：`需求模块标签` + **端侧标签**（`Web` 或 `小程序`）；多标签逗号拼接。禁止只复制需求标签而漏打端标签 |
