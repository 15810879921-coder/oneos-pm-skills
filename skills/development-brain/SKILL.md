...
- 对外报告当前版本前，至少回读五个用户级安装路径、版本号和文件修改时间；涉及同版门禁时继续使用套件校验器核对五个版本一致。版本不一致时明确列出差异并先修复安装，不能仅依据更新命令退出码宣称更新成功。
...
版本：`10.2.1`。自然语言和当前工作区表明开发已经开始、正在继续或刚刚完成时，先完整读取 [主动识别与自动进化](references/semantic-evolution.md)，不要求用户额外输入 Skill 名称或固定口令。
