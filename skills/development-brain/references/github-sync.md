...
4. 仅对新增候选、候选验证或状态提升修改 `skills/development-brain/knowledge/` 的必要文件；运行 Skill 校验，并用 `build-dual-client-packages.ps1 -SkillNames development-brain -Clients codex` 只生成 Codex 包，校验 Codex ZIP 的 SHA-256 与 manifest。按 DB-C-007 暂停维护 Cursor 包和 manifest，历史产物保留不动。
5. 精确暂存本次知识文件、Codex ZIP、Codex manifest 及其直接相关说明；暂存清单不得包含本次生成或修改的 Cursor 路径。提交信息使用中文并包含知识 ID 与动作（新增候选/验证候选/确认规则）；推送 `main` 后读取 `refs/heads/main`，确认提交一致。
