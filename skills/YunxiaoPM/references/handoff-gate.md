...
每个 scope 是独立可交付/可验收的范围；同需求多端分别维护清单。产品更新版本/内容哈希后，旧开发和 QA 回执对该 scope 失效，重新读变更及依赖章节，更新解释并复核；不要求重读无关全仓文档。不得把受影响必需项改为 optional 来省 token。缺失旧资料允许调查、补录、TEMPDEV 隔离验证，不允许冒充产品确认、正式完成开发或正式 QA/生产放行。
