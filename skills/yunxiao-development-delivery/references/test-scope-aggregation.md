...
2. 有精确正式计划、端侧目录和非空具体用例时使用`formal-plan`；开发完成只读取、去重并冻结`directoryIds`与`selectedCaseIds`，不更新 TestHub 结果。
3. 没有正式计划、端侧目录未配置或目录内没有正式用例时使用`mandatory-test-task`；仍创建测试任务，由QA按需求验收点执行并记录结果，不伪造计划或用例。
