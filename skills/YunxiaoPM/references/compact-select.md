...
记录需求 Plan 默认用编号题 + 字母选项；用户可用一行压缩答复确认。命令中的项目 ID 已经官方唯一回读时，第 2 题改为只读回显并可从压缩串省略。
...
| 缺题 | 项目 ID 已官方唯一回读时允许缺第2题；其他题仅在参数已唯一锁定时可省略，否则停并追问 |
| 多答同题 | 后写覆盖先写 |
| 非法字符 | 停，重贴模板 |
...
- 类型、优先级、标签对应项标 ★，并给出建议压缩串；用户可直接改字母或整段重答。
- 仅项目名称预填时仍不得跳过项目题；只有项目 ID 经官方唯一回读后，项目题才改为只读并允许省略 `2x`。
...
1. 记录需求进 Plan 时输出尚未锁定的选择题；项目 ID 已官方唯一回读时，回显项目名+ID并省略第2题。
2. 收到压缩串先解析回显，再等「执行」。
