...
产品交接是死门禁（10.2.10）：`evidence.handoffEvidence`、实时产品 manifest、必读资料哈希、开发理解回执、范围与交付版本全部通过后，才能执行本次流程任何写入。缺失、损坏、过期、不一致或不可读均停止，不降为警告；旧预检、历史已提交代码、已有测试任务、人工状态和催办确认都不能豁免。即使发生在可选工时阶段的写前校验中，交接失败也不得被当作工时失败跳过。补齐资料后重新预检。
...
计划使用`oneos.complete-development-plan/v1`，套件版本为`10.2.10`。生成计划前必须通过只读CLI（TestHub计划读取默认使用受控官方JSON适配器）冻结以下事实：
