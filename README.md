...
| `AutoRDO` | 将碎片化聊天记录、语音转写文本拆解为清晰标题与书面描述；对齐 OneOS 标准模块/部门用词（`oneos-domain.md`），去口头禅、末尾去句号、标注待确认 | 见下方 |
...
3. AI 先对齐 OneOS 领域词典，再输出 **标题**（标准模块 + 核心诉求）与 **描述**（原文转译/清洗结果，无结尾句号），并梳理待确认事项。
