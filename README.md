...
| `AutoRDO` | 将碎片化聊天记录、语音转写文本拆解并提炼为规范标题（`xxxx模块xxxx功能`）与书面描述（原文转译，去口头禅、末尾去句号、标注待确认） | 见下方 |
...
将碎片化文字、聊天记录或录音转写文本，在**保留原意**前提下自动拆解为规范**标题**（格式：`xxxx模块xxxx功能`）与**描述**（原文转译结果，书面表达），供入库写入云效需求。
...
3. AI 将输出拆解后的 **标题**（`xxxx模块xxxx功能`）与 **描述**（原文转译/清洗结果，无结尾句号），并梳理待确认事项。
