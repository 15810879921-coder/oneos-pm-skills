...
| 待确认 | 材料含糊处写 `待确认：…`，不臆造；用户要消解时进入 [confirm-pending.md](confirm-pending.md) 的 Plan 逐条确认 |
| **待确认 Plan 确认** | 详见 [confirm-pending.md](confirm-pending.md)：<br>• `SwitchMode` → plan，按需求条号**逐点**确认<br>• **优先选择题**（2–5 项 +「其他/我补充」），选项用 `oneos-domain.md` 标准模块/部门等辅助生成<br>• 也支持用户粘贴**需求描述/补充说明**一次消解多点<br>• 确认后回填描述并移除已消解项；未确认不得写入定稿 |
...
- [ ] 若做了待确认确认：已进 Plan、优先选择题、选项有词典依据；文字补充已映射到对应点；定稿无未确认臆造  
