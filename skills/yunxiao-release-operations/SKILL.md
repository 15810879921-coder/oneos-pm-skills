...
Operate deployment and release evidence while keeping test, production verification, rollback, and product acceptance separate. Suite version: `10.0.0`.
...
- Until all five lifecycle Skills are officially read back at suite version `10.0.0`, release preparation reads new and legacy evidence but does not write a new-format delivery ledger or execute a new-format merge plan. Existing legacy release flow remains available; version skew must not create mixed evidence.
