...
- 独立只读发现使用 `release_read_batch.py` 并行，输入最多 32 个已有精确范围的官方只读调用、并发数最多 8；输出保持输入顺序、每项调用、值和哈希。依赖读取、范围裁决、写动作及其依赖仍保持顺序。
- 同批多个冻结源/目标使用 `validate_release_change_coverage.py collect-batch`；相同仓库和相同完整 SHA 的历史只读取一次。已存在且计划哈希、仓库、冻结源 SHA、目标基线均一致时使用 `reuse-history`，否则重新官方采集。
- 冲突修订或干净候选只把候选 SHA 作为 `candidateRevision` 做官方文件树覆盖核验；候选不是业务源，不重新读取候选和目标的完整提交历史，也不为覆盖核验递归补齐候选提交对象。
...
- 多个组件的流水线定义/运行盘点和终态轮询按组件并发；单组件内分页保持顺序。等待最终一致性时只轮询受影响任务或执行 ID，不重扫项目、迭代或全部流水线。
- 每次轮询最多等待 60 秒并向用户更新；结束后只抓取失败步骤需要的日志区间。
- 每次准备/执行回执记录范围发现、代码历史、候选覆盖、合并、生产流水线、状态收尾的耗时与官方调用数；性能优化不得省略门禁，只能复用不可变证据、合并重复读取或并行无依赖读取。
