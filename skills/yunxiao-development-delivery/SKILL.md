...
Operate development tasks and Codeup assets without using code activity as a substitute for real work state. Suite version: `7.8.1`.
...
7. Resolve exactly one project user with role `测试主管`, create or reuse the unique `【测试】` task, force its owner to that user's ID, upsert and verify its `测试建议` and `临时需求变更点` handoff description, move the requirement from `开发完成` to `待测试`, and produce the formal test handoff.
...
- Creating or reusing `【测试】` and moving the requirement `开发完成 → 待测试` are two separate verified actions. Exactly one project `测试主管`, owner-role equality, the formal relation, and a read-back verified managed description containing both `测试建议` and `临时需求变更点` are mandatory test-handoff gates.
