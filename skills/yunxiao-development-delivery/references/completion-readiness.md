...
- `recoveryNeeded`可包含`managed_test_scope`。旧输入中的`formal_handoff`仅转为待产品补齐的`warnings`，从恢复阻塞项中移除；仅缺产品交接格式不得加入`knownBlockers`。真实代码范围冲突或验证失败仍是阻塞项。
...
- `recovery_required / recover_then_complete`：正式历史任务已经找到可核验候选，但代码映射或既有测试任务的受管范围尚未补齐。展示精确候选集合；代码归属无法由任务编号、已有台账或正式关系自动证明时只确认一次。确认后按[历史代码交付恢复](historical-code-recovery.md)补录并重新分类，不能直接关单。
