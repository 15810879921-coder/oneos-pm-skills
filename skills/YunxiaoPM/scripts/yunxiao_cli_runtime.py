#!/usr/bin/env python3
"""Secret-safe runtime shared by YunxiaoPM official CLI adapters."""
...
def require_auth_env() -> dict[str, bool]:
    ...
    if not flags["organizationId"] and not flags["apiBaseUrl"]:
        raise AdapterError("缺少云效组织ID或Region版API地址。")
    return flags
...
def current_user(executable: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or not value.get("id"):
        raise AdapterError("PAT用户回读失败。")
    return {"id": str(value["id"]), "name": value.get("name") or value.get("nickName")}
