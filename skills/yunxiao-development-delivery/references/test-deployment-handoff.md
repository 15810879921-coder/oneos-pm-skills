...
2. 按每个适用Codeup仓库的精确代码源和目标分支反查已经创建好的Flow流水线；名称必须带`test/测试`且不带`prod/生产`。每条不同流水线最多启动或关联一次，禁止创建、复制、更新或重命名流水线。
3. 证明部署版本包含本需求所有必需Commit/MR；隔离快速模式必须精确包含开放MR的远端源提交。只收到流水线提交成功、运行中、未知或环境不符均阻塞。
4. 在【测试】描述中幂等写入并回读：
...
4. 所有分组合并后，按涉及仓库执行各自已创建的test流水线；相同流水线去重后只执行一次，并以组合证据证明部署版本覆盖全部回流修复。
