...
9. 校验日期和正数工时；校验快照绑定关联需求编号并含任务目标、本次范围、页面与交互、业务规则、验收条件、来源与版本；校验技术方案绑定同一需求及快照哈希，并含实现范围、处理逻辑、实施步骤、验证标准。冻结需求描述哈希、快照哈希、方案哈希、对象、关系、优先级、字段、状态和负责人指纹。
...
3. 依次幂等维护描述中的`## 开发需求快照`和`## 技术实施方案`，删除旧的本Skill生成的`## 下一阶段`命令块，保留区块外人工内容。
4. 通过CLI创建或复用`PARENT→交付`和`ASSOCIATED→需求`，逐条回读。
5. 聚合回读开发任务负责人、优先级、日期、工时、状态、两条关系、快照哈希和技术方案哈希；新建任务的优先级标识ID必须与交付任务完全一致。
