...
【交付】任务 = 该需求的端侧交付容器
  - 交付端=Web|小程序 → 恰好 1 条
  - 交付端=共用服务 → 恰好 2 条（Web + 小程序，双端交付）
【分析】/【设计】 = YunxiaoPM 在对应端侧交付下建的阶段子项
【开发】/【测试】 = 另属开发 Skill / 测试 Skill，不进本包
ASSOCIATED = 横向挂钩（【交付】 ↔ 需求；双端时两条交付均 ASSOCIATED→同一需求）
SUB / TASK_SUB = 纵向拆解（端侧交付 → 该端分析/设计）
```
...
| 交付端=Web\|小程序 却出现多条交付，或交付端=共用服务 却不是恰好 Web+小程序 各一 | 列出编号请人合并/补齐后再继续 |
| 功能模块标签用「分析/设计/交付」 | 模块标签打在需求（可选交付）上 |

## 任务标题（仅展示）

- 单端：`【交付】` + 需求标题；双端：`【交付】【Web】` / `【交付】【小程序】` + 需求标题
- 单端：`【分析】` / `【设计】` + 需求标题；双端：对应加 `【Web】` / `【小程序】` 前缀
- **端侧标签（强制）**：每条【交付】建单后打显示名 `Web` 或 `小程序`（与该条所属端一致）；可与需求模块标签并存。单端、双端均须打，避免列表肉眼混淆。详见 [settled-rules.md](settled-rules.md)
- 标题可改，**不参与查重**。
