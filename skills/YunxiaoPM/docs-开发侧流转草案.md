...
| 找计划 | 按**精确需求编号**查找正式测试计划；无计划时记录`mandatory-test-task` |
| 执行 | 开发侧先完成实现验证；正式测试始终由对应【测试】任务承接 |
...
|---|---|
| 触发 | `完成开发` +（建议带）开发任务编号 |
| 新建 | 每个非取消【开发】创建或复用唯一【测试】任务，并记录`developmentTaskId` |
| 关联 | 必须同时满足子项 →【交付】、ASSOCIATED→需求 |
| 需求 | → **待测试** |
