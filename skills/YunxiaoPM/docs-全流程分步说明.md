...
|---|---|
| **谁** | AI（开发完成后触发） |
| **做什么** | 开发侧完成与范围相称的自测；按**精确需求编号**查询正式测试计划只用于确定后续测试用例来源，不能决定是否创建测试任务 |
| **需求状态** | 保持 **开发中/开发完成**；开发自测不代替正式测试任务 |
...
|---|---|
| **谁** | 开发人员（自测也完成） |
| **做什么** | 每个非取消【开发】都创建或复用唯一对应的 **【测试】**；有正式计划使用其当前端范围，无计划使用`mandatory-test-task`，都必须由QA完成 |
| **需求状态** | 首个测试任务开始后 → **测试中**；仅全部开发任务均唯一映射已完成测试任务后 → **测试完成** |
| **口令例** | `完成开发：ONEOS-d` |
| **为什么** | 开发交棒给测试；测试侧开始接单 |
...
|---|---|
| **谁** | 测试（谢佳伟等） |
| **做什么** | 打开当前范围【测试】任务；确认关联的需求、交付和端侧范围；计划按需求编号绑定，用例包按`[Web]`/`[小程序]`/`[跨端]`分组 |
| **需求状态** | 首个正式范围开始执行后改为 **测试中** |
...
|---|---|
| **谁** | 测试 / AI 辅助判定 |
| **做什么** | 当前范围用例全绿先收口本范围【测试】；所有承诺范围（含跨端）闭环后才通知发版对接人 |
| **需求状态** | 仅全部范围聚合通过后 → **测试完成** ★ |
| **注意** | 只卡本需求，不卡同迭代里别的需求 |
