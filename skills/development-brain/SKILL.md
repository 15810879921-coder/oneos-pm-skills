...
3. **结束复盘**：在本次最终 Git/验证/MR 证据已经形成后，读取 [候选复盘](knowledge/retrospective-candidates.md)。仅当“现象/根因/修复或规避/验证/适用范围”都有可追溯证据，且通过相同/近似模式排除与 GitHub 发布预检时，才可形成新的 `candidate`；否则输出“无可沉淀候选”。不得因复盘再修改代码、状态、MR、流水线或生产环境。
...
- 先检索全部 `confirmed` 与 `candidate` 记录。命中相同或近似的现象、根因、预防动作和适用范围组合时，一律不新增、不补充、不上传；在当次回执标注命中的记录 ID 和相似依据即可。
...
- 相同/近似排除、GitHub 发布预检或推送/读回失败时，不创建或修改大脑知识；标记“候选未落盘”，保留待写内容和失败原因，禁止声称已沉淀。
...
- 相同或近似模式已经存在时，当前问题不再入脑；即使新增了佐证，也只留在本次任务回执，避免知识库重复膨胀。
