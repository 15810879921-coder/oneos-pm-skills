...
计划使用`oneos.complete-development-plan/v1`，套件版本为`10.1.2`。生成计划前必须通过只读CLI冻结以下事实：
...
`preflight`先实时读取`suite-state`中的五个安装路径，要求版本全部为`10.1.2`，再验证总计划并为各阶段生成通用网关计划和预检回执。`apply`会再次实时回读安装路径。任何关键阶段预检失败时零写入；只有可选工时阶段可以标记跳过。
