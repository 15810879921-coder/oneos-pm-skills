...
Operate deployment and release evidence while keeping test, production verification, rollback, and product acceptance separate. Suite version: `9.4.0`.
...
- `准备发布` requires one unique iteration and creates/reuses one top-level release task with a frozen scope, source delivery list, qualified Bug list, and active pipeline mapping. Missing, cross-project, title-only, ambiguous, delivery-child, or scope-drifting relations block all writes. Missing approver, window, rollback plan, or production-verification plan is recorded as `待补充` and does not block preparation; it still blocks `执行发布`.
